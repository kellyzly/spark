...
import org.apache.spark.crypto.{CryptoStreamUtils, IOUtils}
import org.apache.spark.{SparkConf, Logging}

/**
* A Random implementation that uses random bytes sourced from the
* operating system.
* current not use. If need , please implement
* org.apache.spark.crypto.OpensslAesCtrCryptoCodec#setConf(org.apache.spark.SparkConf)
*          klass = conf.getClass(
...
class OsSecureRandom(conf:SparkConf) extends Random with Closeable with Logging {
  ...
  var randomDevPath: String = CryptoStreamUtils.getRandomDevPath
